//
//  IssueDetailDelayViewController.swift
//  ONULHOI
//
...
extension IssueDetailDelayViewController: StoryboardLoadable {
    static var ownerStoryboard = UIStoryboard(name: "Issue", bundle: nil)
}

class IssueDetailDelayViewController: UMViewController {
...
extension IssueDetailDelayViewController: UITableViewDelegate, UITableViewDataSource {
    func numberOfSections(in tableView: UITableView) -> Int {
        return items.count
    }
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return items[section].skuIssueShortQuantityDelays?.count ?? 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let item = items[indexPath.section].skuIssueShortQuantityDelays?[indexPath.row]
        let cell: IssuDetailDelayContentsCell = tableView.dequeueReusableCell(for: indexPath)
        cell.lb_Title.text = item?.name
        cell.lb_Contents.text = item?.reasonDetail
        return cell
    }
