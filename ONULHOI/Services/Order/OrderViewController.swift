...
extension OrderViewController: StoryboardLoadable {
    static var ownerStoryboard = UIStoryboard(name: "Order", bundle: nil)
}

class OrderViewController: UMViewController {
...
//#endif
    ...
    func reload() {
        var sender = MRest.Sender(path: .operationInventoriesOrderCount, method: .get)
        sender.customUrl = PPCST.Rest.baseURL + PPCST.Rest.apiUri + MRest.Path.operationInventoriesOrderCount.rawValue +
        ...
        MRest.shared.request(sender) { (resp: MRest.Resp<NARow<NDHome.OperationInventoryCountResponse>>) in
            ...
            self.freshItems.insert(NDCommListItem.init(listType: .header, title: HSTR("발주 수량"), count: totalFreshCount), at: 0)
            self.btn_FreshDetail.isHidden = totalFreshCount <= 0
            
            if item.dailyOrderType == "확정" && item.dailyOrderStatus == "UNREAD" {
                self.freshItems.append(NDCommListItem.init(listType: .button(title: HSTR("발주 내역 확인하기"))))
            }
            ...
            self.keepItems.insert(NDCommListItem.init(listType: .header, title: HSTR("발주 수량"), count: totalKeepCount), at: 0)
            ...
            if totalKeepCount > 0 {
                ...
                self.keepItems.append(NDCommListItem.init(listType: .button(title: HSTR("발주 내역 확인하기"))))
            } else {
...
//        let vc = WeekOrderViewController.instantiate()
